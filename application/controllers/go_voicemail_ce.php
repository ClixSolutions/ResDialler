	...
	function go_update_voicemail_list()
	{
		...
		switch($action)
		{
			case "activate":
				...
				$this->commonhelper->auditadmin('ACTIVE','Activated Voicemail(s): '.$this->uri->segment(4));
				break;
			case "deactivate":
				$query = $this->db->query("UPDATE vicidial_voicemail SET active='N' WHERE voicemail_id IN ('$voicemails')");
				$this->commonhelper->auditadmin('INACTIVE','Deactivated Voicemail(s): '.$this->uri->segment(4));
				break;
			case "delete":
