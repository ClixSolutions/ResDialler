...
//		$this->config->initialize($config);
	...
	function go_campaign_list()
	{
		...
		$query = $this->db->query("SELECT pause_code,vpc.campaign_id from vicidial_pause_codes as vpc, vicidial_campaigns as vc where vpc.campaign_id=vc.campaign_id order by pause_code;");
	...
	function go_campaign_wizard()
	{
		...
		else
		{
			if ($campType=='back')
			{
				...
				$query = $this->db->query("SELECT campaign_id,campaign_name,campaign_vdad_exten FROM vicidial_campaigns ORDER BY campaign_id");
				$data['campaigns'] = $query->result();
		...
		foreach ($_FILES as $fileName => $file)
		{
		    if ( ! empty($file['name']))
		    {
				...
				else
				{
					...
					copy($config['upload_path'] . $file['name'],"$upload_path/GO_" . $file['name']);
					chmod("$upload_path/GO_" . $file['name'], 0777);
					$result .= "SUCCESS: File ".$file['name']." uploaded.\n";
					$fn .= "$fileName\n";
					$files .= "GO_".str_replace(".wav",'',$file['name'])." ";

					$this->go_campaign->rsync_file($file['name']);
				}
		    }
		}
