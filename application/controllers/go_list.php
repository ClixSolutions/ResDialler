...
//echo "</script>";        
					...
					if($customlist=="view") { 
							...
							$clist = $this->golist->getcustomlist($ul);
							$data['clist'] = $clist;
...
//		$pc_recs = mysql_num_rows($rslt);
    ...
    function download(){
		...
		$data["url"] = "{$listidx}.csv";
                $dimez .= "lead_id,entry_date,modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner\n";
		$dlquery = $this->golist->dlquery($listidx);
 
                foreach($dlquery as $dlqueryinfo){
     	          $dimez .= "{$dlqueryinfo->lead_id},{$dlqueryinfo->entry_date},{$dlqueryinfo->modify_date},{$dlqueryinfo->status},{$dlqueryinfo->user},{$dlqueryinfo->vendor_lead_code},{$dlqueryinfo->source_id},{$dlqueryinfo->list_id},{$dlqueryinfo->gmt_offset_now},{$dlqueryinfo->called_since_last_reset},{$dlqueryinfo->phone_code},{$dlqueryinfo->phone_number},{$dlqueryinfo->title},{$dlqueryinfo->first_name},{$dlqueryinfo->middle_initial},{$dlqueryinfo->last_name},{$dlqueryinfo->address1},{$dlqueryinfo->address2},{$dlqueryinfo->address3},{$dlqueryinfo->city},{$dlqueryinfo->state},{$dlqueryinfo->province},{$dlqueryinfo->postal_code},{$dlqueryinfo->country_code},{$dlqueryinfo->gender},{$dlqueryinfo->date_of_birth},{$dlqueryinfo->alt_phone},{$dlqueryinfo->email},{$dlqueryinfo->security_phrase},{$dlqueryinfo->comments},{$dlqueryinfo->called_count},{$dlqueryinfo->last_local_call_time},{$dlqueryinfo->rank},{$dlqueryinfo->owner}\n";
