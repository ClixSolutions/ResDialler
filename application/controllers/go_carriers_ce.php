...
class Go_carriers_ce extends Controller {
	...
	{
		parent::Controller();
		$this->load->model(array('go_auth','go_dashboard','go_carriers'));
...
//		$this->config->initialize($config);
	...
	function go_get_carrier()
	{
        ...
        function sippy_register(){
            ...
            if($_POST['captcha'] !== $_SESSION['captcha']){
                 ...
                 $havesippy = $this->go_carriers->db->get('justgovoip_sippy_info');
                 if($havesippy->num_rows() > 0){
                     echo "Error: Multiple GoAutoDial carrier entries are not allowed";
                     die;
                 }
                 ...
                 if(!in_array($r,array(402,400,412,490,500))){
                     ...
                     $data['vicidial_server_carriers']['carrier_id'] = "govoip{$a['authname']}";
                     ...
                     $data['justgovoip_sippy_info']['carrier_id'] = "govoip{$a['authname']}";
                     ...
                     echo "Success: Account creation successful. Please check your email for instructions on how to login to our VoIP web portal";
                 } else {
        ...
        function get_server_ip(){
             ...
             $servers = $this->go_carriers->db->get_where('severs',array('active'=>'Y'))->result(); 
	...
	function go_carrier_wizard()
	{
		...
		if (strlen($action) < 1)
		{
			...
			if ($action == "add_new_carrier")
			{
				...
				if (!$isExist)
				{
					if ($reg_string=="dal.justgovoip.com" || $reg_string=="208.43.27.84")
					{
						...
						$this->db->insert('justgovoip_sippy_info',$data);
					}
					$varSQL = rtrim($varSQL,",");
					$valSQL = rtrim($valSQL,",");
			...
			if ($action == "delete_carrier")
			{
				...
				$query = $this->db->query("DELETE FROM vicidial_server_carriers WHERE carrier_id = '$carrier'");
                                ...
                                $sippy = $this->go_carriers->db->get('justgovoip_sippy_info');
                                if($sippy->num_rows() > 0){
        ...
        function checkInfo(&$amount=null){
                   ...
                   $this->go_carriers->db->where(array('carrier_id'=>$_POST['carrier']));
                   $sippy = $this->go_carriers->db->get('justgovoip_sippy_info');
                   if($sippy->num_rows() > 0){
                        include("/var/www/html/sippysignup/xmlrpc/xmlrpc.inc");
                        $row = $sippy->result();
        ...
        function checkJustgovoip($internalCheck=null,&$response=null){
            if(!empty($_POST)){
				if((!empty($internalCheck) && !is_null($internalCheck))){
					$this->go_carriers->db->where(array('carrier_id'=>$internalCheck));
				}else{
					$this->go_carriers->db->where(array('carrier_id'=>$_POST['carrier']));
				}
				$justgovoip = $this->go_carriers->db->get('justgovoip_sippy_info');
	...
	function go_update_carrier_list()
	{
		...
		switch($action)
		{
			case "activate":
				...
				$this->commonhelper->auditadmin('ACTIVE','Activated Carrier(s): '.$this->uri->segment(4));
				break;
			case "deactivate":
				$query = $this->db->query("UPDATE vicidial_server_carriers SET active='N' WHERE carrier_id IN ('$carriers')");
				$this->commonhelper->auditadmin('INACTIVE','Deactivated Carrier(s): '.$this->uri->segment(4));
				break;
			case "delete":
...
// 			echo "<script>javascript: window.location = '$base'</script>";
        ...
        function copycarrier(){
            if(!empty($_POST)){
                ...
                $isSippy = $this->go_carriers->db->get("justgovoip_sippy_info")->num_rows();
