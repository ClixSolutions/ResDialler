	...
	// end auto create
	    ...
	    if ($stage == "upload") {
    		...
    		$audiofile_dir = preg_replace("/ /",'\ ',$audiofile_dir);
                $audiofile_dir = preg_replace("/@/",'\@',$audiofile_dir);
                $audiofile_name = preg_replace("/ /",'',"go_".$audiofile_name);
    		...
    		$stmt = "SELECT server_id,server_ip,active,server_description FROM servers";
      		...
      		$row = $servers->row();
      		$server_ip = $row->server_id; 
    		$active = $row->active;
    		$server_description = $row->server_description;

    		if(!preg_match('/WEB SERVER/i',$server_description) && $active == "Y"){
		 ...
		 // $listidparam = $this->uri->segment(4);
		  ...
		  // load leads
			...
			$owner_field =				$this->input->post('owner_field');
					...
					$data['clist'] = $clist;		
					$voicefilestable = $this->goaudiostore->getvoicefilestable($data['gouser']);
