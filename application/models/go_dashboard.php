...
class Go_dashboard extends Model {
   ...
   function __construct()
	...
	function go_outbound_today()
	{
	    ...
	    $query = $this->db->query("select count(*) as qresult from vicidial_log where status='$status' and $date $ul");
	    $resultsu = $query->row();
	...
	function go_inbound_today()
	{
	    ...
	    $status = "SALE";
	    $date = "call_date BETWEEN '$query_date 00:00:00' AND '$query_date 23:59:59'";
	    $query = $this->db->query("select count(*) as qresult from vicidial_closer_log where status='$status' and $date $ul");
	    $resultsu = $query->row();
	    $fresults = $resultsu->qresult;
	    if ($fresults == NULL)
	    {
	...
	function go_sippy_info() {
	    $stmt = "SELECT carrier_id,username,web_password,authname,voip_password,vm_password,i_account FROM justgovoip_sippy_info;";
...
// 		$query = $this->db->query("select count(*) AS queue from vicidial_auto_calls where status NOT rlike 'XFER' and call_type rlike 'IN' $ul");
        ...
        function go_calls_ringing_today()
	{
	    ...
	    $query = $this->db->query("select count(*) AS ringing from vicidial_auto_calls where status NOT IN('XFER') and call_type = 'OUT' $ul");
            $resultsu = $query->row();
       ...
       function go_total_calls()
	{
	    ...
	    $query_date =  date('Y-m-d');
	    $query = $this->db->query("SELECT count(*) as totcalls from vicidial_users as us, vicidial_log as vlog, vicidial_list as vl where us.user=vlog.user and vl.phone_number=vlog.phone_number and vl.lead_id=vlog.lead_id and vlog.call_date between '$query_date 00:00:00' and '$query_date 23:59:59'");
            $resultsu = $query->row();
       ...
       function go_total_dialable_leads()
	{
	    ...
	    else
	    {
	       ...
	       $ul = " and campaign_id IN ('$stringv') ";
             ...
             $stringv = $this->go_getall_allowed_campaigns();
             $query = $this->db->query("select count(*) qresult from vicidial_lists as vls,vicidial_list as vl where vl.list_id=vls.list_id and active='Y' and status IN ('N','BUSY','A','DEC','DROP','ERI','NANQUE','NEW','NP','TIMEOT','XDROP') $ul");
             $resultsu = $query->row();
	     $fresults = $resultsu->qresult;
	     if ($fresults == NULL)
	     {
       ...
       function go_hopper_leads_warning_less_h()
	{
	  ...
	  $query = $this->db->query("SELECT COUNT(vh.campaign_id) as mycnt, vl.campaign_id FROM vicidial_hopper as vh    RIGHT OUTER JOIN vicidial_campaigns as vl ON (vl.campaign_id=vh.campaign_id) where vl.active='Y' $ul GROUP BY vl.campaign_id HAVING COUNT(vh.campaign_id) < '100' ORDER BY mycnt DESC , campaign_id ASC;");
