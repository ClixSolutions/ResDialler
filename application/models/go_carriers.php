...
class Go_carriers extends Model {
	...
	{
		$query = $this->db->query("SELECT carrier_id,carrier_name,server_ip,protocol,registration_string,active,user_group FROM vicidial_server_carriers ORDER BY carrier_id;");
		$return = $query->result();
	...
	function go_get_systemsettings()
	{
        ...
        function go_get_govoip(){
             $result = $this->db->get('justgovoip_sippy_info')->result();
             if(empty($result)){ 
