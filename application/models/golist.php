...
class Golist extends Model {
     ...
     function getalllist($wherecampaigns=null,$allowedcampaign=null) {
	$whereto = "AND campaign_id IN ('$allowedcampaign')";
	 //    $stmt = "SELECT vls.list_id,list_name,list_description,(select count(*) from vicidial_list where list_id = vls.list_id) as tally,active,list_lastcalldate,campaign_id,reset_time from vicidial_lists vls,vicidial_list vl where vls.list_id=vl.list_id $whereto group by list_id order by list_id";
	     $stmt = "SELECT list_id,list_name,list_description,(SELECT count(*) as tally FROM vicidial_list WHERE list_id = vicidial_lists.list_id) as tally,active,list_lastcalldate,campaign_id,reset_time from vicidial_lists $wherecampaigns order by list_id";
...
//     }
     ...
     function getcustomlist($whereLOGallowed_campaignsSQL){
			$base = base_url();
