...
class Go_monitoring extends Model {
				...
				if (!eregi("5$|9$|11$|12$", $i)) {
					...
					else
					{
						...
						if ($field=="campaign" || $field=="user" || $field=="mm:ss")
						{
							...
							$onClickField = "onClick=\"updateOrder('$sortorder');\"";
						}
		...
		// 		    	$realtimeHTML .= "<tr style=\"background-color:$bgcolor;color:$color;\" align=center>\n";
					...
					foreach ($fields as $c => $field)
					{
						if (!eregi("3$|4$|5$|7$|8$|9$|11$|12$", $c)) {
							if ($this->userLevel < 9 && eregi("^0$|2$|6$", $c))
							{
								// Do Nothing...
							}
							else
			...
			if ($orderby=='campaignup') {$orderSQL='campaign_id,status,call_time';}
			if ($orderby=='campaigndown') {$orderSQL='campaign_id desc,status desc,call_time desc';}
			...
			if ($campaign_allow_inbound > 0)
				{
				$stmt="select closer_campaigns from vicidial_campaigns where campaign_id IN ('$stringv');";
				...
				$stmt="select status,phone_number,call_type,UNIX_TIMESTAMP(call_time) as 'call_time',campaign_id from vicidial_auto_calls where ( (call_type='IN' and campaign_id IN($closer_campaignsX)) or (campaign_id IN ('$stringv') and call_type rlike 'OUT') ) ORDER BY $orderSQL LIMIT 2000;";
				}
			else
				{
	
				$stmt="select status,phone_number,call_type,UNIX_TIMESTAMP(call_time) as 'call_time',campaign_id from vicidial_auto_calls where campaign_id IN ('$stringv') ORDER BY $orderSQL LIMIT 2000;";
				}
	...
	{
		$query = $this->db->query("SELECT campaign_id,campaign_name,active FROM vicidial_campaigns ORDER BY campaign_id;");
		$return['list'] = $query->result();

		return $return;
	}
	
	function go_dropped_call_list()
	{
		$NOW = date("Y-m-d");
		$xx = 0;
		$query = $this->db->query("SELECT vcs.campaign_id AS campaign_id,campaign_name,calls_today,'' AS dropped_percentage,answers_today AS 'answered',drops_today AS 'dropped' FROM vicidial_campaign_stats vcs, vicidial_campaigns vc WHERE vcs.campaign_id=vc.campaign_id AND update_time BETWEEN '$NOW 00:00:00' AND '$NOW 23:59:59' ORDER BY vcs.campaign_id");
		
		$return = "<thead><tr style='font-weight:bold'>";
		foreach ($query->list_fields() AS $field)
		{
			$return .= "<th style='cursor: pointer;'>".strtoupper(str_replace("_"," ",$field))."</th>";
		}
