...
class Go_reports extends Model {
   ...
   function __construct()
	...
	function go_get_campaigns()
	{
		...
		$query = $this->db->query("SELECT campaign_id,campaign_name FROM vicidial_campaigns WHERE active='Y' $ul ORDER BY campaign_id");
		$return = $query->result();
	...
	{
		$userID = $this->session->userdata('user_name');
		$query = $this->db->query("select num_seats from a2billing_wizard where account_num='$userID'");
		$resultsu = $query->row();
	...
	function go_get_logins()
	{
		...
		{
			$query = $this->db->query("SELECT user,pass FROM vicidial_users WHERE user_group='$userID' AND user_level='1' ORDER BY user");
		}
		else
		{
			$query = $this->db->query("SELECT login,phones.pass FROM phones,vicidial_users WHERE login=REPLACE(user,'_','') AND vicidial_users.user_group='$userID' AND user_level='1' ORDER BY login");
		}
		$datacount = $query->num_rows();
		$dataval   = $query->result();
		$return['datacount']=$datacount;
	...
	function go_get_reports()
	{
		...
		if ($campaignID!='null' || $pageTitle == 'call_export_report')
		{
			...
			if ($pageTitle!='inbound_report') {
				$query = $this->db->query("select campaign_name from vicidial_campaigns where campaign_id='$campaignID'");
			} else {
				$query = $this->db->query("select group_name as campaign_name from vicidial_inbound_groups where uniqueid_status_prefix='".$return['groupId']."'");
			}
			$resultu = $query->row();
			...
			if ($pageTitle=='stats')
			{
				if ($return['request']=='daily') {
					...
					// Total Calls Made
					//$query = $this->db->query("select * from vicidial_log where campaign_id='$campaignID' and length_in_sec>'0' and call_date between '$fromDate 00:00:00' and '$toDate 23:59:59'");
					$query = $this->db->query("select cdate, sum(Hour0) as 'Hour0', sum(Hour1) as 'Hour1', sum(Hour2) as 'Hour2', sum(Hour3) as 'Hour3', sum(Hour4) as 'Hour4', sum(Hour5) as 'Hour5', sum(Hour6) as 'Hour6', sum(Hour7) as 'Hour7', sum(Hour8) as 'Hour8', sum(Hour9) as 'Hour9', sum(Hour10) as 'Hour10', sum(Hour11) as 'Hour11', sum(Hour12) as 'Hour12', sum(Hour13) as 'Hour13', sum(Hour14) as 'Hour14', sum(Hour15) as 'Hour15', sum(Hour16) as 'Hour16', sum(Hour17) as 'Hour17', sum(Hour18) as 'Hour18', sum(Hour19) as 'Hour19', sum(Hour20) as 'Hour20', sum(Hour21) as 'Hour21', sum(Hour22) as 'Hour22', sum(Hour23) as 'Hour23' from (select date_format(call_date, '%Y-%m-%d') as cdate,sum(if(date_format(call_date,'%H') = 00, 1, 0)) as 'Hour0',sum(if(date_format(call_date,'%H') = 01, 1, 0)) as 'Hour1',sum(if(date_format(call_date,'%H') = 02, 1, 0)) as 'Hour2',sum(if(date_format(call_date,'%H') = 03, 1, 0)) as 'Hour3',sum(if(date_format(call_date,'%H') = 04, 1, 0)) as 'Hour4',sum(if(date_format(call_date,'%H') = 05, 1, 0)) as 'Hour5',sum(if(date_format(call_date,'%H') = 06, 1, 0)) as 'Hour6',sum(if(date_format(call_date,'%H') = 07, 1, 0)) as 'Hour7',sum(if(date_format(call_date,'%H') = 08, 1, 0)) as 'Hour8',sum(if(date_format(call_date,'%H') = 09, 1, 0)) as 'Hour9',sum(if(date_format(call_date,'%H') = 10, 1, 0)) as 'Hour10',sum(if(date_format(call_date,'%H') = 11, 1, 0)) as 'Hour11',sum(if(date_format(call_date,'%H') = 12, 1, 0)) as 'Hour12',sum(if(date_format(call_date,'%H') = 13, 1, 0)) as 'Hour13',sum(if(date_format(call_date,'%H') = 14, 1, 0)) as 'Hour14',sum(if(date_format(call_date,'%H') = 15, 1, 0)) as 'Hour15',sum(if(date_format(call_date,'%H') = 16, 1, 0)) as 'Hour16',sum(if(date_format(call_date,'%H') = 17, 1, 0)) as 'Hour17',sum(if(date_format(call_date,'%H') = 18, 1, 0)) as 'Hour18',sum(if(date_format(call_date,'%H') = 19, 1, 0)) as 'Hour19',sum(if(date_format(call_date,'%H') = 20, 1, 0)) as 'Hour20',sum(if(date_format(call_date,'%H') = 21, 1, 0)) as 'Hour21',sum(if(date_format(call_date,'%H') = 22, 1, 0)) as 'Hour22',sum(if(date_format(call_date,'%H') = 23, 1, 0)) as 'Hour23' from vicidial_log where length_in_sec>'0' and date_format(call_date, '%Y-%m-%d') between '$fromDate' and '$toDate' $ul group by cdate $MunionSQL) t group by cdate;");
					$return['data_calls'] = $query->result();
					
					$query = $this->db->query("select phone_number from vicidial_log vl where length_in_sec>'0' and date_format(call_date, '%Y-%m-%d') between '$fromDate' and '$toDate' $ul $TunionSQL");
					$return['total_calls'] = $query->num_rows();
					
					// Total Number of Leads
					$query = $this->db->query("select * from vicidial_list as vl, vicidial_lists as vlo where vlo.campaign_id='$campaignID' and vl.list_id=vlo.list_id");
					$return['total_leads'] = $query->num_rows();
					
					// Total Number of New Leads
					$query = $this->db->query("select * from vicidial_list as vl, vicidial_lists as vlo where vlo.campaign_id='$campaignID' and vl.list_id=vlo.list_id and vl.status='NEW'");
					$return['total_new'] = $query->num_rows();
					
					// Total Agents Logged In
					$query = $this->db->query("select date_format(event_time, '%Y-%m-%d') as cdate,count(user) as cuser from vicidial_agent_log where campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') between '$fromDate' and '$toDate' group by user");
					$return['total_agents'] = $query->num_rows();
					$return['data_agents'] = $query->result();
					
					// Disposition of Calls
					$query = $this->db->query("select status, sum(ccount) as ccount from (select status,count(*) as ccount from vicidial_log vl where length_in_sec>'0' and date_format(call_date, '%Y-%m-%d') between '$fromDate' and '$toDate' $ul group by status $DunionSQL) t group by status;");
				...
				if ($return['request']=='weekly') {
					...
					// Total Calls Made
					//$query = $this->db->query("select * from vicidial_log where campaign_id='$campaignID' and length_in_sec>'0' and call_date between '$fromDate 00:00:00' and '$toDate 23:59:59'");
					$query = $this->db->query("select weekno, sum(Day0) as 'Day0', sum(Day1) as 'Day1', sum(Day2) as 'Day2', sum(Day3) as 'Day3', sum(Day4) as 'Day4', sum(Day5) as 'Day5', sum(Day6) as 'Day6' from (select week(DATE_FORMAT( call_date, '%Y-%m-%d' )) as weekno, sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 0, 1, 0))  as 'Day0', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 1, 1, 0))  as 'Day1', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 2, 1, 0))  as 'Day2', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 3, 1, 0))  as 'Day3', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 4, 1, 0))  as 'Day4', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 5, 1, 0))  as 'Day5', sum(if(weekday(DATE_FORMAT( call_date, '%Y-%m-%d' )) = 6, 1, 0))  as 'Day6' from vicidial_log where length_in_sec>'0' and week(DATE_FORMAT( call_date, '%Y-%m-%d' )) between week('$fromDate') and week('$toDate') $ul group by weekno $MunionSQL) t group by weekno;");
					$return['data_calls'] = $query->result();
					
					$query = $this->db->query("select phone_number from vicidial_log vl where length_in_sec>'0' and week(DATE_FORMAT( call_date, '%Y-%m-%d' )) between week('$fromDate') and week('$toDate') $ul $TunionSQL");
					$return['total_calls'] = $query->num_rows();
					...
					// Total Number of New Leads
					$query = $this->db->query("select * from vicidial_list as vl, vicidial_lists as vlo where vlo.campaign_id='$campaignID' and vl.list_id=vlo.list_id and vl.list_id='NEW'");
					$return['total_new'] = $query->num_rows();
					
					// Total Agents Logged In
					$query = $this->db->query("select date_format(event_time, '%Y-%m-%d') as cdate,count(user) as cuser from vicidial_agent_log where campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') between '$fromDate' and '$toDate' group by cdate");
					$return['total_agents'] = $query->num_rows();
					$return['data_agents'] = $query->result();
					
					// Disposition of Calls
					$query = $this->db->query("select status, sum(ccount) as ccount from (select status,count(*) as ccount from vicidial_log vl where length_in_sec>'0' and week(DATE_FORMAT( call_date, '%Y-%m-%d' )) between week('$fromDate') and week('$toDate') $ul group by status $DunionSQL) t group by status;");
					$return['total_status'] = $query->num_rows();
					$return['data_status'] = $query->result();
				}
				...
				if ($return['request']=='monthly') {
					...
					// Total Agents Logged In
					$query = $this->db->query("select date_format(event_time, '%Y-%m-%d') as cdate,count(user) as cuser from vicidial_agent_log where campaign_id='$campaignID' and MONTH(event_time) between MONTH('$fromDate') and MONTH('$toDate') group by cdate");
					$return['total_agents'] = $query->num_rows();
					$return['data_agents'] = $query->result();
					
					// Disposition of Calls
					$query = $this->db->query("select status, sum(ccount) as ccount from (select status,count(*) as ccount from vicidial_log vl where length_in_sec>'0' and MONTH(call_date) between MONTH('$fromDate') and MONTH('$toDate') $ul group by status $DunionSQL) t group by status;");
					$return['total_status'] = $query->num_rows();
					$return['data_status'] = $query->result();
				}
			}
			...
			if ($pageTitle=="agent_detail") {
				...
				$query = $this->db->query("SELECT full_name,user FROM vicidial_users ORDER BY user LIMIT 100000");
				...
				$query = $this->db->query("SELECT user,SUM(login_sec) AS login_sec FROM vicidial_timeclock_log WHERE event IN('LOGIN','START') AND date_format(event_date, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' GROUP BY user LIMIT 10000000");
				...
				$user_count=0;
				$i=0;
				$query = $this->db->query("SELECT user,SUM(pause_sec) AS pause_sec,sub_status FROM vicidial_agent_log WHERE date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND pause_sec > 0 AND pause_sec < 65000 $ul GROUP BY user,sub_status ORDER BY user,sub_status DESC LIMIT 10000000");
				...
				$query = $this->db->query("SELECT user,wait_sec,talk_sec,dispo_sec,pause_sec,lead_id,status,dead_sec FROM vicidial_agent_log WHERE date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' $ul LIMIT 10000000");
				...
				while ( ($m < $uc) and ($m < 50000) )
					...
					$TCuserAUTOLOGOUT = ' ';
					$query = $this->db->query("SELECT COUNT(*) as cnt FROM vicidial_timeclock_log WHERE event='AUTOLOGOUT' AND user='$Suser[$m]' AND date_format(event_date, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate'");
					$timeclock_ct = $query->num_rows();
			...
			if ($pageTitle == "agent_pdetail") {
				...
				$query = $this->db->query("select count(*) as calls,sum(talk_sec) as talk,full_name,vicidial_users.user as user,sum(pause_sec) as pause_sec,sum(wait_sec) as wait_sec,sum(dispo_sec) as dispo_sec,status,sum(dead_sec) as dead_sec from vicidial_users,vicidial_agent_log where date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' and vicidial_users.user=vicidial_agent_log.user and pause_sec<65000 and wait_sec<65000 and talk_sec<65000 and dispo_sec<65000 group by user,full_name,status order by full_name,user,status desc limit 500000");
			...
			if ($pageTitle=="dispo") {
				...
				if (isset($list_ids) && $list_ids[0] == "ALL") {
					$query = $this->db->query("SELECT list_id FROM vicidial_lists WHERE campaign_id='$campaignID' ORDER BY list_id");
				...
				$query = $this->db->query("SELECT status,status_name from vicidial_statuses order by status");
				...
				$query = $this->db->query("SELECT status,status_name from vicidial_campaign_statuses where campaign_id='$campaignID' order by status");
				...
				$query = $this->db->query("SELECT status, if(called_count >= 10, 10, called_count) as called_count, count(*) as count from vicidial_list where list_id IN('".implode("','",$list_ids)."') and status NOT IN('DC','DNCC','XDROP') group by status, if(called_count >= 10, 10, called_count) order by status,called_count");
	...
	//					}
			...
			if ($pageTitle == "sales_agent") {
				...
				// Outbound Sales
				$query = $this->db->query("SELECT us.full_name AS full_name, us.user AS user,
						SUM(IF(vlog.status = 'SALE', 1, 0)) AS sale
				...
				// Inbound Sales
				$query = $this->db->query("SELECT closer_campaigns FROM vicidial_campaigns WHERE campaign_id='".$campaignID."' ORDER BY campaign_id");
				$row = $query->row();
			...
			if ($pageTitle == "sales_tracker") {
				...
				if ($return['request']=='outbound') {
					$query = $this->db->query("select distinct(vl.phone_number) as phone_number,vlo.call_date as call_date,us.full_name as agent,
			...
			if ($pageTitle == "inbound_report") {
				$query = $this->db->query("SELECT * FROM vicidial_closer_log WHERE campaign_id = '$campaignID' AND date_format(call_date, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate'");
				$TOPsorted_output = $query->result();
			...
			if ($pageTitle == "call_export_report") {
				...
				$query = $this->db->query("SELECT campaign_id FROM vicidial_campaigns");
				...
				$query = $this->db->query("SELECT list_id FROM vicidial_lists");
				...
				$query = $this->db->query("select custom_fields_enabled from system_settings");
				...
				// 			exit;
					...
					while ($k > $i)
						...
						$ex_list_name='';
						$ex_list_description='';
						$query = $this->db->query("SELECT list_name,list_description FROM vicidial_lists where list_id='$export_list_id[$i]'");
						...
						$ex_status_name='';
						$query = $this->db->query("SELECT status_name FROM vicidial_statuses where status='$export_status[$i]'");
						$ex_list_ct = $query->num_rows();
						if ($ex_list_ct > 0)
							{
							$row = $query->row();
							...
							{
							$query = $this->db->query("SELECT status_name FROM vicidial_campaign_statuses where status='$export_status[$i]'");
							$ex_list_ct = $query->num_rows();
							if ($ex_list_ct > 0)
								{
								$row = $query->row();
						...
						if ( (($rec_fields=='ID') or ($rec_fields=='FILENAME') or ($rec_fields=='LOCATION') or ($rec_fields=='ALL')) && $i > 0 )
							...
							$rec_filename='';
							$rec_location='';
							$query = $this->db->query("SELECT recording_id,filename,location from recording_log where vicidial_id='$export_vicidial_id[$i]' order by recording_id desc LIMIT 10");
						...
						if ($export_fields=='EXTENDED')
							...
							if (strlen($export_uniqueid[$i]) > 0)
								...
								$query = $this->db->query("SELECT caller_code,server_ip from vicidial_log_extended where uniqueid LIKE \"$uniqueidTEST%\" and lead_id='$export_lead_id[$i]' LIMIT 1");
								...
								$query = $this->db->query("SELECT hangup_cause,dialstatus,channel,dial_time,answered_time from vicidial_carrier_log where uniqueid LIKE \"$uniqueidTEST%\" and lead_id='$export_lead_id[$i]' LIMIT 1");
								...
								$query = $this->db->query("SELECT result from vicidial_cpd_log where callerid='$export_call_id[$i]' LIMIT 1");
						...
						if ($call_notes=='YES')
							...
							if (strlen($export_vicidial_id[$i]) > 0)
								{
								$query = $this->db->query("SELECT call_notes from vicidial_call_notes where vicidial_id='$export_vicidial_id[$i]' LIMIT 1");
						...
						if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
							...
							if ($export_entry_list_id[$i] > 99)
								{$CF_list_id = $export_entry_list_id[$i];}
							$query = $this->db->query("SHOW TABLES LIKE \"custom_$CF_list_id\"");
							$tablecount_to_print = $query->num_rows();
							if ($tablecount_to_print > 0) 
								{
								$query = $this->db->query("describe custom_$CF_list_id");
								...
								if ($columns_ct > 1)
									{
									$query = $this->db->query("SELECT * from custom_$CF_list_id where lead_id='$export_lead_id[$i]' limit 1");
			...
			if ($pageTitle=="dashboard") {
				...
				$query = $this->db->query("select us.user,us.full_name,val.status,count(*) as calls from vicidial_users as us,vicidial_agent_log as val where date_format(val.event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' and us.user=val.user and val.status<>'' and val.campaign_id='$campaignID' group by us.user,us.full_name,val.status order by us.full_name,us.user,val.status desc limit 500000");
				foreach ($query->result() as $row)
				{
					$agent[$row->user][$row->status] = $row->calls;
				}
	
				$query = $this->db->query("select val.status from vicidial_agent_log as val, vicidial_log as vl where val.status<>'' and date_format(val.event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' and val.campaign_id='$campaignID' and val.uniqueid=vl.uniqueid group by val.status limit 500000");
				foreach ($query->result() as $i => $row)
				{
				...
				if (count($agent)>0)
				{
					$query = $this->db->query("select lower(us.user) as user,us.full_name from vicidial_users as us, vicidial_agent_log as val where date_format(val.event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' and lower(us.user)=lower(val.user) and val.campaign_id='$campaignID' group by us.user limit 500000");
	...
	//			$TOPsorted_output .= "<tr><td nowrap style=\"border-top:#D0D0D0 dashed 1px;\" colspan=\"".(1+$t)."\"><div align=\"right\" class=\"style3\"><strong>&nbsp;TOTAL:&nbsp;</strong></td><td style=\"border-top:#D0D0D0 dashed 1px;\"><div align=\"center\" class=\"style3\"><strong>&nbsp;$total_all&nbsp;</strong></td></tr>";
				...
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status<>''");
				$total_calls = $query->num_rows();
				
				// TOTAL CONTACTS ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status IN ('$statuses_codes')");
				$total_contacts = $query->num_rows();
			
				// TOTAL NON-CONTACTS ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status NOT IN ('$statuses_codes')");
				$total_noncontacts = $query->num_rows();
	
				// TOTAL SALES ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status IN ('SALE','XFER')");
				$total_sales = $query->num_rows();
			
				// TOTAL XFER ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status='XFER'");
				$total_xfer = $query->num_rows();
			
				// TOTAL NOT INTERESTED ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status='NI'");
				$total_notinterested = $query->num_rows();
			
				// TOTAL CALLBACKS ====  AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)
				$query = $this->db->query("SELECT * FROM vicidial_agent_log WHERE campaign_id='$campaignID' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND status='CALLBK'");
				$total_callbacks = $query->num_rows();
				
				$query = $this->db->query("select sum(talk_sec) talk_sec,sum(pause_sec) pause_sec,sum(wait_sec) wait_sec,sum(dispo_sec) dispo_sec,sum(dead_sec) dead_sec from vicidial_users,vicidial_agent_log where date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' and vicidial_users.user=vicidial_agent_log.user and $dac_agents_query2 talk_sec<36000 and wait_sec<36000 and talk_sec<36000 and dispo_sec<36000 and campaign_id='$campaignID' limit 500000");
...
// 				$total_dialer_calls_output[]='';
				...
				foreach ($statuses_code as $code) {
					$query = $this->db->query("select count(*) as cnt from vicidial_log where campaign_id='$campaignID' and length_in_sec>'0' and status='$code' and date_format(call_date, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate'");
					$row_out[$code]=$query->row()->cnt;
					
					$query = $this->db->query("select count(*) as cnt from vicidial_closer_log where campaign_id IN ('$inbCamp') and length_in_sec>'0' and status='$code' and date_format(call_date, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate'");
...
// 				$total_dialer_calls_output = json_encode($total_dialer_calls_output);
				...
				foreach ($statuses as $status) {
					$query = $this->db->query("SELECT count(*) as cnt FROM vicidial_agent_log WHERE campaign_id='$campaignID' and status='$status' and date_format(event_time, '%Y-%m-%d') BETWEEN '$fromDate' AND '$toDate' AND (sub_status NOT LIKE 'LOGIN%' OR sub_status IS NULL)");
	...
	function go_get_groupid()
	{
      ...
      function go_getall_closer_campaigns()
      {
		$campaignID = $this->uri->segment(6);
	    $query_date =  date('Y-m-d');
	    $query = $this->db->query("select trim(closer_campaigns) as qresult from vicidial_campaigns where campaign_id='$campaignID' order by campaign_id"); 
	    $resultsu = $query->row();
	...
	{
		$groupId = $this->go_get_groupid();
		//$query = $this->db->query("select group_id,group_name from vicidial_inbound_groups where uniqueid_status_prefix='$groupId';");
	...
	function go_get_statuses($camp)
		{
		# grab names of global statuses and statuses in the selected campaign
		$query = $this->db->query("SELECT status,status_name,selectable,human_answered from vicidial_statuses order by status");
		$statuses_to_print = $query->num_rows();
		...
		$query = $this->db->query("SELECT status,status_name,selectable,human_answered from vicidial_campaign_statuses where campaign_id='$camp' and selectable='Y' and human_answered='Y' order by status");
		$Cstatuses_to_print = $query->num_rows();
	...
	{
		$query = $this->db->query("SELECT local_call_time AS call_time FROM vicidial_campaigns WHERE campaign_id='$camp'");
		$call_time = $query->row()->call_time;

		if (strlen($call_time) > 0)
		{
			$query = $this->db->query("SELECT ct_default_start, ct_default_stop FROM vicidial_call_times WHERE call_time_id='$call_time'");
