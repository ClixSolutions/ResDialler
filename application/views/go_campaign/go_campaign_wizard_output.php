...
$(document).ready(function()
{
	...
	$('#uploadform').ajaxForm({
		...
		complete: function(xhr) {
			...
			if (response[0]=='wav')
			{
			    ...
			    document.getElementById('fileNames').innerHTML = response[3].replace(' ',':');
			    for (x in fname)
			    {
					if (fname[x]!='')
	...
	$('#upload').click(function()
	{
		...
		{
			alert('Please include a <strong>.WAV</strong> file.');
		}
		else
		{
	...
	$('#more').click(function()
	{
		...
		if (wavFileNum<=10)
		{
			$('#survey_audio').append('<tr><td style="text-align:center"><input type="file" name="wavFile'+wavFileNum+'" id="wavFile'+wavFileNum+'" value="" /></td></tr>');
...
?>
	    <table id="survey_audio" style="width:100%;">
            	...
            	<tr>
                	<td style="text-align:center"><input type="file" name="wavFile1" id="wavFile1" value="" /></td>
                </tr>
            </table>
            </form>
            <br />
