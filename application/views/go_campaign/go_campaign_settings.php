...
{
	$camp_script_list .= '<option value="'.$script->script_id.'">'.$script->script_id.' - '.$script->script_name.'</option>';
}
...
$(document).ready(function()
{
	...
	$('#campaign_script option').each(function()
	{
		if ($(this).val() == '<?php echo $campinfo->campaign_script; ?>')
			$('#campaign_script').val('<?php echo $campinfo->campaign_script; ?>');
	});
	...
	$('#saveSettings').click(function()
	{
		...
		if ($('#campaign_vdad_exten').val() != '8366' && $('#campaign_vdad_exten').val() != '8373')
		{
			...
			var web_form_address = $('#web_form_address').val().replace('//','dslash');
			var web_form_address = web_form_address.replace('?','qmark');
			var repweb_form_address = web_form_address.replace('/','sslash');
		...
		else
		{
			...
			var basicArray = [survey_first_audio_file, survey_method, status, number_of_lines, campaign_cid, local_call_time, '', campaign_vdad_exten, active, repweb_form_address, dial_prefix];
		}
		...
		if (isAdvance > 0)
		{
			//console.log($('#force_reset_hopper').val());
			$.each($('.advanceSettings'), function()
			{
...
//		$('#overlayContent').fadeOut("slow").load('<?php echo $base; ?>index.php/go_campaign_ce/go_modify_settings/'+campaign_id+'/modify/'+basicArray+'/'+advArray+'/'+isAdvance+'/'+allowed_inbgrp+'/'+notallowed_inbgrp).fadeIn("slow");
	...
	$('#view_hopper').click(function()
	{
		...
		$('#hopperBox').css({'width': '850px','margin-left': 'auto', 'margin-right': 'auto', 'left': '14%', 'right': '14%', 'padding-bottom': '20px'});
		$('#hopperBox').animate({
			top: "75px"
		}, 500);
...
?>
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Force Reset Leads on Hopper:</td><td><select id="force_reset_hopper" name="force_reset_hopper" class="advanceSettings"><option value="N">N</option><option value="Y">Y</option></select></td>
    </tr>
	<tr class="advance_settings" style="display:none;">
...
?>
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Manual Dial Prefix:</td><td><?=form_input('manual_dial_prefix',$campinfo->manual_dial_prefix,'id="manual_dial_prefix" maxlength="20" size="20" class="advanceSettings"') ?></td>
    </tr>
<?php
// 	<tr class="advance_settings" style="display:none;">
...
?>
	...
	<tr>
    	<td style="text-align:right;" nowrap>Script:</td><td><select id="campaign_script"><option>NONE</option><?php echo $camp_script_list; ?></select></td>
    </tr>
	<tr class="advance_settings" style="display:none;">
		...
		while (false !== ($file = readdir($handle))) {
			if($file != '..' && $file != "."){
				...
				if($filename == "go_"){
					...
					echo "<tr><td>&raquo; <a id='$ctr' href='#'>$fname</a>\n";
					echo "<script>
							$('#$ctr').click(function (){
