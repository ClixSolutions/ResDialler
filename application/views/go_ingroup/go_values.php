...
/* start lists */
	...
	if($action=="editcallmenufinal") {
		...
		if(is_null($sucesshint)) {
			...
			for($x=3;$x<14;$x++) {
				...
				$submititemsplit = explode('=', $itemsumitexplode[$x]);
			...
			if(strlen($stmtx)){
				$this->goingroup->editvalues($stmtx);
                        ...
                        for($i=0;$i<14;$i++){
                            ...
                            for($j=14;$j<count($itemsumitexplode);$j++){
	...
	if($action=="deletedid") {
			$listiddel = $this->input->post('didid_delete');

			
			if(is_null($sucesshint)) {
				$stmt="DELETE from vicidial_inbound_dids where did_id='$listiddel' limit 1;";
	...
	if($action=="deletecallmenu") {
			$callmenudel = $this->input->post('callmenu_delete');
	...
	if ($action == "showcallmenuoption")
	{
		...
		switch ($routeid)
		{
			case "CALLMENU":
				...
				$optionsHTML .= "<td colspan=\"6\" style=\"text-align:center;\"><label>Call Menu: </label>".form_dropdown('option_route_value_'.$ctr,$callmenuArray,null,'id="option_route_value_'.$ctr.'"')."</td>";
				break;
			case "INGROUP":
				$stmt="SELECT group_id,group_name FROM vicidial_inbound_groups ORDER BY group_id";
				$ingrouppulldown = $this->asteriskDB->query($stmt);
	...
	if ($action == "showoption")
	{
		...
		switch ($routeid)
		{
			case "CALLMENU":
				...
				$hiddenOptionHTML .= "<label>Call Menu: </label>".form_dropdown('option_route_value_'.$ctr,$callmenuArray,$getcallmenuoptions[0]->option_route_value,'id="option_route_value_'.$ctr.'"');
				break;
			case "INGROUP":
				$stmt="SELECT group_id,group_name FROM vicidial_inbound_groups ORDER BY group_id";
				$ingrouppulldown = $this->asteriskDB->query($stmt);
