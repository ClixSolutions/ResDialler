...
if ($action == 'vm_list') {
	...
	/*	$rslt = $this->asteriskDB->query($stmt);
		...
		$stmt="SELECT voicemail_id,fullname,email from phones where active='Y' order by voicemail_id";
		...
		foreach ($rslt3->result_array() as $rowx) {
		    ...
		    $finalHTML .= "<select id=vmfilesdrop  onchange=\"java_script_:setDivVal('$comments',this.options[this.selectedIndex].value)\">";
			$finalHTML .= "<option value=\"$defaultvalue\" selected>-- Default Value --</option>";
			$finalHTML .= $vmfiles;
			$finalHTML .= "</select>";
	...
	if ($allowed_user < 1)
       ...
       if ($ss_conf_ct > 0) {
		...
		if ($sounds_central_control_active < 1)
			...
			$stmt="SELECT moh_id,moh_name,random from vicidial_music_on_hold where active='Y' and moh_id='$user' order by moh_id";
			$rslt2 = $this->asteriskDB->query($stmt);
