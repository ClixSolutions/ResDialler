...
function agentMonitoring()
{
    ...
    // 	if ($(this).is(':hidden'))
    $('#boxRealTime').show();
    $('#boxRealTime').css({'width': '800px', 'margin-left': '50%', 'left': '-400px', 'padding-bottom': '20px'});
    $('#boxRealTime').animate({
...
function updateOrder(order,type)
{
	...
	{
		$('#monitor_camp_banner').show();
		$('#overlayContentRealTime').load('<? echo $base; ?>index.php/go_site/go_monitoring/'+isExpanded+'/'+order+'/agents/'+selectedCampaign);
	} else {
		$('#monitor_camp_banner').hide();
...
$(function()
{
		...
		$('.monitor_camp').change(function()
		{
			...
			//$('#overlayContentRealTime').html('<center><img src="<? echo $base; ?>img/goloading.gif" style="opacity:.65;" /></center>');
			$('#overlayContentRealTime').load('<? echo $base; ?>index.php/go_site/go_monitoring/'+isExpanded+'/'+orderByRealtime+'/'+monitoringType+'/'+selectedCampaign);
		});
});
...
#closeboxRealTime,#closeboxAgent{
	float:right;
	width:26px;
	height:26px;
...
<!--									<p>GO Analytics is basically a projection of your production performance based on your current server statistics, agent performance, leads quality, etc.</p>
			...
			<div id="right-widget" class="postbox-container" style="width:49.5%;">
				<div class="meta-box-sortables ui-sortable">
					...
					<!-- SERVER'S STATISTICS -->
					<div id="dashboard_server_statistics" class="postbox ">
						<div class="handlediv" title="Click to toggle"><br></div>
						...
						<div class="inside">
							...
							<div class="table table_filesystems" id="table_filesystems"></div>
                                        ...
                                        <!-- Control Panel-->
					<div id="dashboard_controls" class="postbox ">
						<div class="handlediv" title="Click to toggle"><br></div>
...
<div id="monitor_camp_banner" style="float: right;font-size: 11px;">Select a Campaign: <?=form_dropdown('monitor_agents',$camp_array,$selected_campaign,'id="monitor_agents" class="monitor_camp" style="font-size: 11px;"') ?></div>
...
<div id="overlayAgent" style="display:none;z-index:99;"></div>
<div id="boxAgent" style="display:none;z-index:100;">
...
<table id="test" border=0 cellpadding="3" cellspacing="3" style="width:100%;">
    ...
    <tr>
    	<td style="text-align:right;font-weight:bold;" nowrap>Phone Pass:</td><td><input type="text" id="users_phone_pass" name="users_phone_pass" value="" readonly="readonly" /></td>
    </tr>
    <tr>
