...
$(function()
{
	...
	$('#add_phone').click(addNewPhones);
	
	// Tool Tip
	$(".toolTip").tipTip();
	
	$('#table_container').load('<? echo $base; ?>index.php/go_phones_ce/go_update_phone_list/');
});
...
function addNewPhones()
{
	...
	//$('#overlayContent').fadeOut("slow").load('<? echo $base; ?>index.php/go_phones_ce/go_phone_wizard/').fadeIn("slow");
...
function delPhone(phone)
{
	...
	if (answer)
	{
		$.post("<?=$base?>index.php/go_phones_ce/go_phone_wizard", { exten: phone, action: "delete_phone" },
		function(data){
			if (data=="DELETED")
			{
