...
# 120427-1725 - Fixed 3-way logging issue
#
...
if (isset($_GET["chklogin"]))				{$chklogin=$_GET["chklogin"];}
	elseif (isset($_POST["chklogin"]))		{$chklogin=$_POST["chklogin"];}
if (isset($_GET["phone_login"]))			{$VU_phone_login=$_GET["phone_login"];}
	elseif (isset($_POST["phone_login"]))		{$VU_phone_login=$_POST["phone_login"];}
if (isset($_GET["phone_pass"]))			{$VU_phone_pass=$_GET["phone_pass"];}
	elseif (isset($_POST["phone_pass"]))		{$VU_phone_pass=$_POST["phone_pass"];}
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		echo "</select><br>\n";
		echo "<font size=2 color=red><b>*REQUIRED: USERNAME/PASSWORD.</b></font>\n";
		exit;
		}
	else
		{
		$stmt="SELECT user_group,user_level,agent_shift_enforcement_override,shift_override_flag,active from vicidial_users where user='$user' and pass='$pass'";
		if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
		$rslt=mysql_query($stmt, $link);
		...
		if($VU_active==""){
			...
			echo "<option value=\"invalid\">-- INVALID --</option>\n";
			echo "</select><br>\n";
			echo "<font size=2 color=red><b>INVALID USERNAME/PASSWORD</b></font>\n";
			exit;
		}
		...
		if($VU_active=="N") {
			...
			echo "</select><br>\n";
			echo "<font size=2 color=red><b>AGENT: IN-ACTIVE</b></font>\n";
			exit;
		}

		//error catch phone ative account = N

		$alias_found=0;
		$stmt="select count(*) from phones_alias where alias_id = '$VU_phone_login';";
		$rsltalias=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01018',$VD_login,$server_ip,$session_name,$one_mysql_log);}
		$alias_ct = mysql_num_rows($rsltalias);
		if ($alias_ct > 0)
			{
			$row=mysql_fetch_row($rsltalias);
			$alias_found = "$row[0]";
			}
		if ($alias_found > 0)
			{
			$stmt="select alias_name,logins_list from phones_alias where alias_id = '$VU_phone_login' limit 1;";
			$rslt=mysql_query($stmt, $link);
			...
			if ($alias_ct > 0)
				...
				$row=mysql_fetch_row($rslt);
				$alias_name = "$row[0]";
				$VU_phone_login = "$row[1]";
				}
			}

		$pa=0;
		if ( (eregi(',',$VU_phone_login)) and (strlen($VU_phone_login) > 2) )
			{
			$phoneSQL = "(";
			$phones_auto = explode(',',$VU_phone_login);
			...
			while($pa < $phones_auto_ct)
				...
				$phoneSQL .= "(login='$phones_auto[$desc]' and pass='$VU_phone_pass')";
				$pa++;
				}
			$phoneSQL .= ")";
			}
		else {$phoneSQL = "login='$VU_phone_login' and pass='$VU_phone_pass'";}

		$authphone=0;
	      
			
		if((strlen($VU_phone_login < 1)) and (strlen($VU_phone_pass < 1)))
                {
...
// 	if ($chklogin)
// 		{
// 		echo "|$VU_phone_login|$VU_phone_pass|";
// 		}
	...
	if ($affected_rows > 0)
		...
		#			}
			...
			if (strlen($list_id)>0)
				...
				### check if there is a custom list fields for a specific list id
				$stmt = "SELECT field_id FROM vicidial_lists_fields WHERE list_id='$entry_list_id';";
				$rslt = mysql_query($stmt, $link);
				$custom_field_count = mysql_num_rows($rslt);
...
#			}
		...
		// Check if list_id has custom fields
		$stmtC="SELECT vlf.field_id FROM vicidial_lists_fields AS vlf, vicidial_list AS vl WHERE vls.list_id=vl.list_id AND vl.lead_id='$lead_id';";
		$rsltC=mysql_query($stmtC, $link);
		$numC=mysql_num_rows($rsltC);
...
if ($ACTION == 'CalLBacKLisT')
	...
	while ($callbacks_count>$loop_count)
		{
		$stmt = "SELECT first_name,last_name,phone_number,gmt_offset_now,state,alt_phone from vicidial_list where lead_id='$lead_id[$loop_count]';";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
		...
		echo "$row[0] ~$row[1] ~$row[2] ~$callback_id[$loop_count] ~$lead_id[$loop_count] ~$campaign_id[$loop_count] ~$status[$loop_count] ~$entry_time[$loop_count] ~$callback_time[$loop_count] ~$comments[$loop_count] ~$PHONEdialable ~$row[5]\n";
		$loop_count++;
		}
