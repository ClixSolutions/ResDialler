...
if(isset($_POST['SaveWebForm']))
{
   ...
   $agentDetails  = $diallerDB->query_first("SELECT
                                              ...
                                              ,(
                                                ...
                                                LEFT JOIN
                                                  vicidial_list AS VLIST ON VLOG.lead_id = VLIST.lead_id
                                                LEFT JOIN
                                                  vicidial_users AS VUSR ON VLOG.user = VUSR.user
                                                WHERE
                                                  VLOG.lead_id = VL.lead_id
                                                AND
                                                  VLOG.status IN ('SALE', 'PPIREF')
                                                ORDER BY
                                                  VLOG.call_date DESC
                                                LIMIT 1
                                              ) AS agentFullName
                                            FROM
                                              vicidial_list AS VL
                                            WHERE
                                              VL.lead_id = " . (int)$_GET['lead_id'] . "
                                            LIMIT 1
                                           ");
     */
     $agentDetails = $diallerDB->query_first("SELECT
                                                 VXL.user AS agent
                                                ,VU.user_group AS agentGroup
     ...
     {
       // -- Found for GAB (1-Tick)
       // -------------------------
       $data['LeadRef2'] = "1TICK";
     }
     else if(in_array($agentDetails['agentGroup'], $agentGroup_Resolv))
     {
       // -- Found for Resolve
       // --------------------
       $data['LeadRef2'] = "RESOLVE";
     }
     else if(in_array($agentDetails['agentGroup'], $agentGroups_GBS))
     {
       // -- Found for GBS
       // ----------------
       $data['LeadRef2'] = "1TICK-GBS";
     }
     else
     {
       // -- Not found, so set Not Known
       // ------------------------------
       $data['LeadRef2'] = 'NOT-KNOWN';
     }
    ...
    $diallerDB->query_write("UPDATE
                               gab_log_senior_transfer
                             SET
                                lead_source = '" . mysql_escape_string($data['ListName']) . "'
                               ,tele_agent = '" . mysql_escape_string($data['AgentFullName']) . "'
                               ,status = 2
                             WHERE
                               lead_id = " . (int)$_GET['lead_id'] . "
                             LIMIT 1
...
<html>
  <head>
    ...
    <script type="text/javascript">
      function saveForm()
      {
        document.Save_to_Debtsolv_Form.submit();
      }
    </script>
	...
	<body>
	  <div id="TopNav">
		  ...
		  <div style="width:450px;text-align:right;float:right;">
			  <form name="Save_to_Debtsolv_Form" id="Save_to_Debtsolv_Form" method="POST" action="<?=$_SERVER['PHP_SELF'];?>?client_id=<?=(int)$_GET['client_id'];?>&list_id=<?=(int)$_GET['list_id'];?>&lead_id=<?=$_GET['lead_id'] . '&client_id=' . $_GET['client_id'] . '&agent=' . $_GET['agent'];?>" style="margin:0px" onsubmit="getCustomerInfoValues();changeProgressStatus('Please Wait')" style="margin:0px;">
			    <?=customer_info_form();?>
          ...
          <?php
          if($saved === false && (int)$debtsolvClientID == 0)
          {
            ?>
    ...
    <div>
      <table cellpadding="5">
        <tr>
          <td colspan="5" align="center"><h2>Client's Creditor Information</h2></td>
        </tr>
        <tr>
				  <th>Creditor Name</th>
				  <th>Type of Credit</th>
        ...
        <?php
        if(isset($creditorList) && count($creditorList) > 0)
        {
          foreach($creditorList as $creditor)
          {
            ?>
            <tr>
              <td><?=$creditor['description'];?></td>
              <td><?=$creditor['credit_type'];?></td>
              <td align="right">&pound;<?=number_format($creditor['amount_owed'], 2);?></td>
              <td align="right">&pound;<?=number_format($creditor['monthly_amount'], 2);?></td>
              <td align="center">
                <?php
                switch($creditor['client_responsible'])
                {
                  case 1:
                    echo 'Client';
                  break;
